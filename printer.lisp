    ...
    (cons
     (with-object-case part 
       ...
       (variable-declaration (qualifier specifier &rest initializers)
        (sformat "~o ~o ~{~o~^, ~};" qualifier specifier initializers))
       ...
       (struct-specifier (identifier &rest declarations)
        (sformat "struct ~o{~{~o~}}" identifier declaration))
       ...
       (switch-statement (expression &rest statements)
        (sformat "switch(~o){~{~%~o~}}" expression statements))
       (case-label (case)
        (if (eql :default case)
            (sformat "default: ")
            (sformat "case ~o: " case)))
       ...
       (do-statement (statement expression)
         (sformat "do~o~%while(~o);" statement expression))
